...
bool HCTL2022::Initialise(StructuredDataI &data) {
    ...
    if (ret) {
        ...
        if (ret) {
            oe = (GPIO_TypeDef *) GetHandle(oeId.Buffer());
            ret = (oe != NULL);
        }
        if (!ret) {
        ...
        if (ret) {
            ...
            if (ret) {
                sel = (GPIO_TypeDef *) GetHandle(selId.Buffer());
                ret = (sel != NULL);
            }
            if (!ret) {
        ...
        if (ret) {
            ...
            if (ret) {
                reset = (GPIO_TypeDef *) GetHandle(resetId.Buffer());
                ret = (reset != NULL);
            }
            if (!ret) {
        ...
        if (ret) {
            ...
            if (ret) {
                byteReg = (GPIO_TypeDef *) GetHandle(regId.Buffer());
                ret = (byteReg != NULL);
            }
            if (!ret) {
        ...
        if (ret) {
            ...
            if (ret) {
                clk = (TIM_HandleTypeDef *) GetHandle(clkId.Buffer());
                ret = (clk != NULL);
            }
            if (!ret) {
