...
bool GPIO::Initialise(StructuredDataI &data) {
    ...
    if (ret) {
        ...
        if (ret) {
            ...
            if(!ret){
                REPORT_ERROR(ErrorManagement::InitialisationError, "Invalid GPIO handler %s", gpioId.Buffer());
            }
        } else {
            REPORT_ERROR(ErrorManagement::InitialisationError, "No GPIO specified");
        }
    }
        ...
        for (uint32 i = 0u; i < numberOfSignals && ret; i++) {
            ...
            if (ret) {
                ...
                if (ret) {
                    ...
                    if (!ret) {
                        REPORT_ERROR(ErrorManagement::InitialisationError,
                                "The GPIO signal must be a scalar");
                    }
                }
            }
        }
