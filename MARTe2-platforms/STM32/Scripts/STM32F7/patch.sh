#!/bin/sh
	...
	grep -q _PATCHED main.c || sed -i 's/osKernelStart();/HAL_TIM_Base_Start(\&_TIMER_HANDLE);\n\r&/' main.c
	...
	grep -q 'CalibrateTimer' stm32f7xx_it.c || (sed -i 's/void NMI_Handler(void)/extern void CalibrateTimer();\n&/' stm32f7xx_it.c && sed -i 's/osSystickHandler();/&\nCalibrateTimer();/' stm32f7xx_it.c)
