...
bool EfficientDriver::PrepareNextState(const char8 * const currentStateNameIn,
	...
	if(ret){
		...
		REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "IsInput=%d", isInput);
	}
	if (ret) {
...
void EfficientDriver::SetWriteBufferPointer(uint64 writeMemIn) {

	//the index changes in the Synchronise fun
	writeMem=(uint8*)writeMemIn;
}
