            ...
            if (ret) {
                ...
                if (ret) {
                    ...
                    while ((unreadInput > 0) && (ret)) {
                        ...
                        if(static_cast<int32>(readSize) > unreadInput){
                            readSize = static_cast<uint32>(unreadInput);
                        }
                        ...
                        if (ret) {
                            for (uint32 k = 0u; k < readSize; k++) {
                                if(buffer[k] == '\0'){
                                    postContent += "&st;";
                                }
                                else{
                                    postContent += buffer[k];
                                }
                            }
                            unreadInput -= static_cast<int32>(readSize);
                        }
    ...
    if (ret) {
        ...
        if (ret) {
            ...
            if (ret) {
                ...
                while ((content.GetLine(line, !headerHandled)) && (ret)) {
